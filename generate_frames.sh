...
for filename in *; do
    ...
    ffmpeg -i "$filename" -vf scale=960:540 -c:v libx264 -preset fast -c:a aac "$filename"_resized.mp4 -hide_banner
...
#    echo "ffmpeg -i $filename"_small.mp4 -r 1/1 $data_root/video_frames/"$filename"/frame_%03d.bmp"
