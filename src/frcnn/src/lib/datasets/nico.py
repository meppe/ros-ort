...
class Nico(imdb):
    ...
    CLASS_PROPERTIES = {'classes': (1, ['banana', 'eggplant', 'apple', 'tomato', 'fish', 'pepper', 'lemon', 'die', 'ball']),
                        'shapes': (1, ['long', 'round', 'squared']),
                        'colors': (1, ['yellow', 'purple', 'green', 'red', 'gray', 'white', 'orange']),
    ...
    def __init__(self, image_set, year, devkit_path=None):
        ...
        self._data_path = os.path.join(self._devkit_path, 'nico' + self._year)
        ...
        assert os.path.exists(self._devkit_path), \
                'nico devkit path does not exist: {}'.format(self._devkit_path)
        assert os.path.exists(self._data_path), \
                'nico data path does not exist: {}'.format(self._data_path)
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'nico' + self._year)
    ...
    def _load_pascal_annotation(self, index):
        ...
        for indx, obj in enumerate(objs):
            ...
            object_cat = obj.find('name').text.lower().strip()
            if object_cat not in self._class_to_ind.keys():
                print("Object class {} not supposed to be in dataset".format(object_cat))
                continue
            else:
                print("Reading data for object class {} from xml".format(object_cat))
            cls = self._class_to_ind[object_cat]
    ...
    def _write_voc_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} nico results file'.format(cls)
...
if __name__ == '__main__':
    print("exec nico main")
    from datasets.nico import Nico
    d = Nico('trainval', '2017')
    res = d.roidb
